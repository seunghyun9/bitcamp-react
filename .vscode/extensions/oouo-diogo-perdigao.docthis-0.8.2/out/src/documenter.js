...
class Documenter {
    ...
    documentThis(editor, commandName, forCompletion) {
        ...
        if (documentNode && documentNode.kind === ts.SyntaxKind.VariableDeclarationList) {
            ...
            documentNode = documentNode.declarations[0];
        }
        if (!documentNode) {
    ...
    _emitAuthor(sb) {
        if (vs.workspace.getConfiguration().get("docthis.includeAuthorTag", false)) {
            const author = vs.workspace.getConfiguration().get("docthis.authorName", "");
    ...
    _emitDate(sb) {
        if (vs.workspace.getConfiguration().get("docthis.includeDateTag", false)) {
            const dateFormatInput = vs.workspace.getConfiguration().get("docthis.dateTagFormat");
            const dateToAppend = dateformat(new Date(), dateFormatInput) || "";
    ...
    _emitParameters(sb, node) {
        if (node.kind === ts.SyntaxKind.PropertyDeclaration) {
            const propertyChildren = node.getChildren();
            const arrowFunction = propertyChildren.find((child) => {
                return child.kind === ts.SyntaxKind.ArrowFunction;
            });
            if (!arrowFunction) {
                return;
            }
            const isArrowFunction = ts.isArrowFunction(arrowFunction);
            if (!isArrowFunction) {
                return;
            }
            this._generateParameters(sb, arrowFunction);
        }
        else {
            if (!node.parameters) {
                return;
